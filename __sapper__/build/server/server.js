...
const css$1$1 = {
	code: "@media only screen and (min-width: 800px){.post--metadata.svelte-1tbjtux{display:grid;grid-template-columns:0.8fr 2.2fr 0.4fr 1fr;-webkit-box-align:center;align-items:center}}.post__tag.svelte-1tbjtux{padding:3px 5px;background-color:#dcdcdc;margin:2px 3px;border-radius:3px;display:inline-block}.post--body.svelte-1tbjtux{margin:0px 10px}pre.svelte-1tbjtux{margin-bottom:1em;padding:2% 4%;width:auto;overflow:auto;font-size:1.4em;line-height:1.3em}img.svelte-1tbjtux{max-width:100%}.center.svelte-1tbjtux{display:block;margin:0 auto;box-shadow:-3px 8px 10px #d8d8d8;width:100%;max-width:-webkit-max-content;max-width:-moz-max-content;max-width:max-content;border-radius:2px}.post--title.svelte-1tbjtux{font-size:36px;margin:15px 0 5px;font-weight:lighter}.post--sub-title.svelte-1tbjtux{font-weight:lighter;font-size:22px;margin-top:10px}.post--metadata.svelte-1tbjtux{margin-top:10px}",
...
<div class="${"content"}"><h1 class="${"post--title svelte-1tbjtux"}">${escape(post.title)}</h1>
  <h4 class="${"post--sub-title svelte-1tbjtux"}">${escape(post.subTitle)}</h4>
  <picture><source${add_attribute("srcset", `https://assets.hectane.com/${post.route}/mobile.webp`, 0)} media="${"(max-width: 420px)"}" type="${"image/webp"}">
    ...
    <img class="${"post-title-image svelte-1tbjtux"}"${add_attribute("src", `https://assets.hectane.com/${post.route}/title.jpg`, 0)}${add_attribute("alt", post.title, 0)}></picture>
  <div class="${"post--metadata svelte-1tbjtux"}">${validate_component(Author, "Author").$$render(
		$$result,
		{
			name: authorData.name,
			avathar: authorData.avathar,
    ...
    <div class="${"post__tags"}">${each(post.tags, tag => `<span class="${"post__tag svelte-1tbjtux"}">${escape(tag)}</span>`)}</div>
    <div class="${"post__views"}">${escape(pageViews)} views</div></div>
  <div class="${"post--body svelte-1tbjtux"}">${each(post.body, ({ type, data }) => `${type === "image"
	? `<picture><img${add_attribute("alt", data.caption, 0)} class="${escape(null_to_empty(`${data.withBorder ? "border " : ""}${data.withBackground ? "background " : ""}${data.stretched ? "stretched" : "center"}`)) + " svelte-1tbjtux"}"${add_attribute("src", `${data.url.replace(/\.[^/.]+$/, "")}.jpg`, 0)}>
        </picture>`
	: ``}
      ${type === "header"
	...
	: ``}
      ${type === "code"
	? `<pre class="${"hljs svelte-1tbjtux"}"><code>${`${highlight(data.code)}`}</code>
        </pre>`
	: ``}
      ${type === "paragraph"
	? `<p>${data.text}
