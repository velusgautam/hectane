...
<div class="${"content"}"><h1 class="${"post--title svelte-11dwihb"}">${escape(post.title)}</h1>
  <h4 class="${"post--sub-title svelte-11dwihb"}">${escape(post.subTitle)}</h4>
  <picture><source${add_attribute("srcset", `https://assets.hectane.com/${post.route}/mobile.webp`, 0)} media="${"(max-width: 420px)"}" type="${"image/webp"}">
    ...
    <img class="${"post-title-image svelte-11dwihb"}"${add_attribute("src", `https://assets.hectane.com/${post.route}/title.jpg`, 0)}${add_attribute("alt", post.title, 0)}></picture>
  <div class="${"post--metadata svelte-11dwihb"}">${validate_component(Author, "Author").$$render(
		$$result,
		{
			name: authorData.name,
			avathar: authorData.avathar,
    ...
    <div class="${"post__tags"}">${each(post.tags, tag => `<span class="${"post__tag svelte-11dwihb"}">${escape(tag)}</span>`)}</div>
    <div class="${"post__views"}">${escape(pageViews)} views</div></div>
  <div class="${"post--body svelte-11dwihb"}">${each(post.body, ({ type, data }) => `${type === "image"
	? `<picture><img${add_attribute("alt", data.caption, 0)} class="${escape(null_to_empty(`${data.withBorder ? "border " : ""}${data.withBackground ? "background " : ""}${data.stretched ? "stretched" : "center"}`)) + " svelte-11dwihb"}"${add_attribute("src", `${data.url.replace(/\.[^/.]+$/, "")}.jpg`, 0)}>
        </picture>`
	: ``}
      ${type === "header"
	...
	: ``}
      ${type === "code"
	? `<pre class="${"hljs svelte-11dwihb"}"><code>${`${highlight(data.code)}`}</code>
        </pre>`
	: ``}
      ${type === "paragraph"
	? `<p>${data.text}
